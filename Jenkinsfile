...
pipeline {
  ...
  stages {
    ...
    sh '''#!/bin/bash
ssh -o StrictHostKeyChecking=no jenkins@34.129.141.103 << 'EOF'
echo '[INFO] Checking node-exporter status...'
if docker ps -a --format '{{.Names}}' | grep -q '^node-exporter$'; then
  echo '[INFO] node-exporter container exists. Restarting it...'
  docker stop node-exporter || true
  docker rm node-exporter || true
fi

echo '[INFO] Pulling node-exporter image...'
docker pull prom/node-exporter

echo '[INFO] Starting node-exporter container...'
docker run -d \
  --name node-exporter \
  -v "/proc:/host/proc:ro" \
  -v "/sys:/host/sys:ro" \
...
echo '[INFO] node-exporter deployed successfully.'
EOF
'''
